...
list_t* insert_at_foot(list_t *list, node_t* data) {
	node_linkedlist_t *new;
	new = (node_linkedlist_t*)malloc(sizeof(*new));
	assert(list!=NULL && new!=NULL);
	new->current_node = data;
	new->next = NULL;
	if (list->foot==NULL) {
...
void free_linked_list(list_t* linked_list){
	node_linkedlist_t* current;
	node_linkedlist_t* previous;
    assert(linked_list);
	current = linked_list->head;
	while(current){
		previous = current;
		current = current->next;
        free(previous->current_node);
		free(previous);
	}
	free(linked_list);
}
